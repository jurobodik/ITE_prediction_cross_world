...
estimate_rho_L <- function(
    ...
    y_col = "Y",
    t_col = "T",
    x_cols = c("X1"), # observed covariates
    z_cols = c("Z1"), #auxiliary covariates only for rho estimation
    x_eval = NULL, # if NULL then constructed automatically as 0.25, 0.5 and 0.75 quantiles of each X column (or most common values if discrete)
    mu_method = c("ranger", "lm"), 
    compute_B_L = TRUE, # if TRUE, also computes B_L and rho_tilde_L
    hA = 0.5,     # bandwidth for A(x) estimation
    hSigma = 0.5, # bandwidth for sigma_t(x) estimation
  ...
  # ----------------------------
  if (!all(c(y_col, t_col, x_cols, z_cols) %in% names(data))) {
    stop("Some of y_col / t_col / x_cols / z_cols are not in data.")
  }
  ...
  # Decide if outcome is binary or continuous
  is_binary <- FALSE
  if (is.logical(Y_raw)) {
    is_binary <- TRUE
  ...
  # Convert Y to numeric for variance computations
  if (is_binary) {
    if (is.logical(Y_raw)) {
      Y <- as.numeric(Y_raw)
    } else if (is.factor(Y_raw) && nlevels(Y_raw) == 2) {
    ...
    if (!(length(uy) <= 2 && all(uy %in% c(0, 1)))) {
      stop("Binary outcome detected/requested, but Y is not 0/1 (or 2-level factor/logical).")
    }
  } else {
    Y <- as.numeric(Y_raw)
  }
  
  # Kernel weighting uses X as numeric matrix
  X_mat <- as.matrix(data[, x_cols, drop = FALSE])
  storage.mode(X_mat) <- "double"
  
  # Model frames with safe column names
  ...
  if (is.null(x_eval)) {
    ...
    uniq_counts <- vapply(X_df_raw, function(col) length(unique(col[!is.na(col)])), integer(1))
    is_non_numeric <- vapply(X_df_raw, function(col) !(is.numeric(col) || is.integer(col)), logical(1))
    ...
    if (is_discrete) {
      ...
      if (p > 5) {
        ...
        qs <- lapply(X_df_raw, function(col) quantile(col, probs = c(0.25, 0.5, 0.75),
                                                      na.rm = TRUE, type = 7))
        x_eval <- expand.grid(lapply(qs, function(v) as.numeric(unique(v))),
    ...
    if (!all(x_cols %in% names(x_eval))) stop("x_eval must contain columns in x_cols.")
  ...
  kernel_weights_X <- function(X, x0, h) {
    ...
    gauss_kernel(sqrt(d2) / h) + 1e-12
  }
  
  # ----------------------------
  # Fit mu models in each arm
  # ----------------------------
  d0 <- subset(dat, T == 0, select = -T)
  d1 <- subset(dat, T == 1, select = -T)
  
  fit0 <- NULL; fit1 <- NULL
  pred_mu0_train <- NULL; pred_mu1_train <- NULL
  ...
  if (mu_method == "ranger") {
    ...
    if (is_binary) {
      ...
      args0 <- list(formula = Y ~ ., data = d0, num.trees = ranger_num_trees,
                    probability = TRUE, respect.unordered.factors = TRUE)
      args1 <- list(formula = Y ~ ., data = d1, num.trees = ranger_num_trees,
                    probability = TRUE, respect.unordered.factors = TRUE)
      if (!is.null(base_rf_seed)) { args0$seed <- base_rf_seed + 101L; args1$seed <- base_rf_seed + 102L }
      ...
      args1 <- list(formula = Y ~ ., data = d1, num.trees = ranger_num_trees,
                    respect.unordered.factors = TRUE)
      if (!is.null(base_rf_seed)) { args0$seed <- base_rf_seed + 111L; args1$seed <- base_rf_seed + 112L }
      fit0 <- do.call(ranger::ranger, args0)
      fit1 <- do.call(ranger::ranger, args1)
  ...
  # Predict mu_t at fixed x0 and all observed Z rows
  predict_mu_at_x <- function(x0_vec) {
    newX <- as.data.frame(matrix(x0_vec, nrow = nrow(Z_df_model), ncol = ncol(X_df_model), byrow = TRUE))
  ...
  # ----------------------------
  # If continuous and compute_B_L: fit sigma_t(x,z) via residual^2 models
  # ----------------------------
  ...
  if (compute_B_L && !is_binary) {
    ...
    if (mu_method == "ranger") {
      argsv0 <- list(formula = r2 ~ ., data = v0_dat, num.trees = ranger_num_trees,
                     respect.unordered.factors = TRUE)
      argsv1 <- list(formula = r2 ~ ., data = v1_dat, num.trees = ranger_num_trees,
                     respect.unordered.factors = TRUE)
      if (!is.null(base_rf_seed)) { argsv0$seed <- base_rf_seed + 201L; argsv1$seed <- base_rf_seed + 202L }
  ...
  # ----------------------------
  # sigma_t(x): local weighted variance of Y within each arm
  # ----------------------------
  ...
  # ----------------------------
  # Output: only the fields you had before
  # ----------------------------
  out <- list(
    x_eval = x_eval,
    mu_method = mu_method,
  ...
  # ----------------------------
  # Confidence intervals: add only CI matrices
  # ----------------------------
  if (ci) {
